...
Poetic Programming is a view of programming that emphasizes the importance of meaning.  Programming is a form of writing, and its primary purpose, as with all writing, is to communicate.  Specifically, the most important goal of any program is to communicate to other programmers what it is trying to do, how it aims to do it, and why this is a good thing to do.  This is more important even than working correctly, because a program that succeeds in its communication mission can be understood, corrected, and improved, while a program that fails to communicate is unmaintainable and destined to fade into irrelevance.

Meaning comes from semantic content -- the words in the program.  This includes comments, keywords and names.  Current programming practices, with some exceptions, relegate meaning to a minor concern or ignore it altogether.  The unfortunate result is that modern software tends to be harder to understand than it neeeds to be, and therefore harder to maintain and improve.  Poetic programming addresses this problem by adopting poetry as a model for programming, not in the superficial sense of rhyming or being formatted like a poem, but in the deeper sense of maximizing meaning.  Poetry does this by being economical (saying a lot with few words), rich (carrying multiple layers of meaning), and aesthetic (being a joy to read).  Poetic Programming says that programs should embody these same values.


##Poetry and Meaning

The attributes of poetry that Poetic Programming seeks to emulate -- economy, richness, and beauty -- are means to an end; the end is to convey meaning.  Human writing in general contains two kinds of meaning: denotational (the dictionary definitions) and affective (the feelings the writing evokes).  Many kinds of writing, such as technical writing, reference works, statutes, contracts, and many others, limit themselves as much as possible to denotational meaning, which can be objectively defined and delineated.  Affective meaning is subjective and much harder to nail down.  But that doesn't mean it's not useful or effective.  Some kinds of writing, such as poetry, actively use affective meaning to great effect.
...
There is another divide, less appreciated but just as consequential, that mirrors the distinction between description and metaphor: the distinction between internal and external semantics of a program.  Internal semantics concern architecture and implementation strategy.  Something might be a `Factory`, a `Repository`, a `Visitor` and so on.  It might be `Abstract`, or `Remote` or even `Mock` (for use in automated tests).
...
Programmers and modern programming practices naturally focus on the internal semantics of a program, for a number of reasons: internal semantics are generalizable across applications, while external semantics vary widely from program to program; internal semantics directly affect performance, robustness and extensibility, while external semantics are generally orthogonal to such concerns; and internal semantics, unlike external semantics, operate in the domain that programmers know best, which is programming.
